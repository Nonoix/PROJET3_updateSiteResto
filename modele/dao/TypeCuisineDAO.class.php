...
class TypeCuisineDAO {
    ...
    public static function deleteTypeCuisine(int $idTC) :bool{
        $ok = false;
        try{
        $requete = "DELETE FROM typeCuisine WHERE idTC = :idTC";
        $stmt = Bdd::getConnexion()->prepare($requete);
        $stmt->bindValue(':idTC', $idTC, PDO::PARAM_INT);
        $ok = $stmt->execute();
        } catch (Exception $e) {
            throw new Exception("Erreur dans la méthode " . get_called_class() . "::deleteTypeCuisine : <br/>" . $e->getMessage());
        }
        return $ok;  
    }
    
    public static function addTypeCuisine(TypeCuisine $lesCuisines) :bool{
        $ok = false;
        try{
        $requete = "INSERT INTO typeCuisine (idTC, libelleTC) VALUES (:idTC,:libelleTC)";
        $stmt = Bdd::getConnexion()->prepare($requete);
