...
class RestoDAO {
    ...
    public static function addResto(Resto $unResto) :bool{
        $ok = false;
        try{
        ...
        $stmt->bindValue(':latitudeDegR', $unResto->getLatitudeDegR(), PDO::PARAM_STR);
    ...
    public static function updateResto(Resto $unResto) :bool{
        $ok = false;
        try{
        $requete = "UPDATE resto SET nomR = :nomR, numAdrR = :numAdrR, voieAdrR = :voieAdrR, cpR = :cpR, villeR = :villeR, descR = :descR, horairesR = :horairesR, lesPhotos = :lesPhotos, lesTypesCuisineProposes = :lesTypesCuisineProposes WHERE idR = :idR";
        $stmt = Bdd::getConnexion()->prepare($requete);
        $stmt->bindValue(':nomR', $unResto->getNomR(), PDO::PARAM_STR);
        $stmt->bindValue(':numAdrR', $unResto->getNumAdr(), PDO::PARAM_STR);
        $stmt->bindValue(':voieAdrR', $unResto->getVoieAdr(), PDO::PARAM_STR);
        ...
        $stmt->bindValue(':descR', $unResto->getDescR(), PDO::PARAM_STR);
        $stmt->bindValue(':horairesR', $unResto->getHorairesR(), PDO::PARAM_STR);
        $stmt->bindValue(':lesPhotos', $unResto->getLesPhotos(), PDO::PARAM_STR);
        $stmt->bindValue(':lesTypesCuisineProposes', $unResto->getLesCritiques(), PDO::PARAM_STR);
        $stmt->bindValue(':idR', $unResto->getIdR(), PDO::PARAM_INT);
        $ok = $stmt->execute();
        } catch (PDOException $e) {
            throw new Exception("Erreur dans la méthode " . get_called_class() . "::updateResto : <br/>" . $e->getMessage());
        }
        return $ok;   
    }
}
